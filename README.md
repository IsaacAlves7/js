...
Como funciona o Javascript? A folha de dicas abaixo mostra as características mais importantes do Javascript.
...
![Uploading Captura de tela 2025-05-12 181746.png…]()
...
É aí que entra a ideia de **componibilidade**. Em vez de espalhar chamadas HTTP pelo código, você cria clientes ou serviços que podem ser combinados, orquestrados e até substituídos. No React, por exemplo, isso aparece naturalmente em hooks como `useUser()`, `useOrders()`, `useKPIs()`, que internamente chamam um client HTTP comum. Em frameworks mais estruturados, isso vira algo muito próximo de um *Repository* ou *Gateway* no sentido do Clean Architecture, só que do lado do front-end.

Outro ponto crucial é que o front-end **não deve refletir cegamente o formato do back-end**. Muitas vezes, a API expõe dados normalizados ou orientados a persistência, enquanto a UI precisa de dados agregados, derivados ou prontos para renderização. Por isso, é comum existir uma camada de *adapters* ou *mappers* no front-end, transformando DTOs da API em ViewModels. Quando essa camada existe, mudar um endpoint ou até trocar REST por GraphQL afeta muito menos o resto da aplicação.

Também entra aqui a questão de **estratégia de dados**, não só de chamadas. Cache, deduplicação, invalidação, *optimistic updates* e controle de estado remoto passam a ser tão importantes quanto a URL do endpoint. Ferramentas como React Query, SWR ou Apollo Client surgem exatamente para tratar consumo de API como um problema de **estado distribuído**, não como simples I/O.
...
**WebSockets**: Permitem comunicação bidirecional em tempo real entre cliente e servidor, mantendo uma única conexão aberta. Funciona como se fosse um cano entre cliente e servidor. É um cliente conectado no servidor em tempo real. É uma conexão real constante! Bibliotecas como SocketIO e SignalR são as mais utilizadas para isso.
